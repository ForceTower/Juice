...
object SagresDemandParser {
    ...
    fun getOffers(document: Document): List<SagresDemandOffer>? {
        ...
        for (element in elements) {
            ...
            val title = element.selectFirst("span")?.text()?.trim() ?: continue
            ...
            for (discipline in disciplines) {
                ...
                val id = discipline.selectFirst("input[type=\"hidden\"]")?.attr("name")?.trim() ?: continue
                val selected = discipline.selectFirst("input[type=\"hidden\"]")?.attr("value")?.trim() ?: continue
